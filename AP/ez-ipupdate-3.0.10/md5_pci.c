...
static void MDPrintBuf (MD5_CTX *mdContext, char *buf, int buflen)
{
  int i;
...
void MDStringBuf (char *inString, char *buf, int buflen)
{
  MD5_CTX mdContext;
  unsigned int len = strlen (inString);
  ...
  MD5Update (&mdContext, inString, len);
  MD5Final (&mdContext);
  MDPrintBuf (&mdContext, buf, buflen);
}
