...
static void dhcprelay_loop(int *fds, int num_sockets, int max_socket, char **clients,
		struct sockaddr_in *server_addr, uint32_t gw_ip, int in_nat) ATTRIBUTE_NORETURN;
static void dhcprelay_loop(int *fds, int num_sockets, int max_socket, char **clients,
		struct sockaddr_in *server_addr, uint32_t gw_ip, int in_nat)
{
...
int dhcprelay_main(int argc, char **argv)
{
...
#if 1   // SJC
        /* dhcprelay [-n] client_iface1,client_iface2,... server_iface [server_IP] */
        int     in_nat = 0;
	...
	/* doesn't return */
	dhcprelay_loop(fds, num_sockets, max_socket, clients, &server_addr, gw_ip, in_nat);
