...
#if 1 //vance 2009.11.03
					else send_NAK(&packet); 
#endif
				} else if (requested) {
					/* INIT-REBOOT State */
...
#else
				if ( requested_align >= server_config.start_ip && requested_align < server_config.end_ip) 
					send_ACK(&packet, requested_align);
				else	
					send_NAK(&packet);
				/* RENEWING or REBINDING State */
#endif
...
#if 1 //vance 2009.11.03
				if ( packet.ciaddr >= server_config.start_ip && packet.ciaddr < server_config.end_ip) 
					send_ACK(&packet, packet.ciaddr);
				else	
					send_NAK(&packet);
#endif
				/* RENEWING or REBINDING State */
