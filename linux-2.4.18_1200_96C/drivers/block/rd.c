...
#if defined(CONFIG_RTK_VOIP) || defined(CONFIG_RTK_VOIP_MODULE) || defined(CONFIG_MTD_RTL8196_SPI_MP)
#ifdef LZMA_COMPRESSED_RAMDISK
#error "LZMA_COMPRESSED_RAMDISK has defined!?"
#endif
#define LZMA_COMPRESSED_RAMDISK // Support LZMA-compressed ramdisk
#endif
...
// david ------------------
//#define CONFIG_LOAD_COMPRESSED_RAMDISK
//#ifndef CONFIG_NINO_8MB
//	#define CONFIG_LOAD_COMPRESSED_RAMDISK
//#endif
//------------------------
...
static void __init rd_load_image(kdev_t device, int offset, int unit)
{
	...
	if (nblocks == 0) {
#ifdef CONFIG_LOAD_COMPRESSED_RAMDISK //sc_yang
...
#ifdef CONFIG_LOAD_COMPRESSED_RAMDISK
...
static int first_read_inbuf(void)
{
	if (exit_code) return -1;
	
	insize = crd_infp->f_op->read(crd_infp, inbuf, 13,
				      &crd_infp->f_pos);
...
#include "../../lib/LzmaStateDecode.h"
#include "../../lib/LzmaStateDecode.c"
#endif
...
#ifdef LZMA_COMPRESSED_RAMDISK
{
	extern unsigned long __rd_start, __rd_end;
	if (first_read_inbuf() < 0)
		printk("LZMA ERROR %d\n", __LINE__);
	//printk("first_read_inbuf() insize = %d\n", insize);
	unsigned char *startBuf=( unsigned char *)inbuf;
	unsigned char *outBuf = (unsigned char *)window;
	unsigned int inLen=( unsigned char *)&__rd_end - ( unsigned char *)&__rd_start;	
	...
	outSizeHigh = le32_to_cpu(outSizeHigh);
		...
		printk("LZMA: Too big uncompressed stream\n");
		return 0;
   	}	
	startBuf += 8;    
	...
	if ( state.Probs == 0)
		printk(KERN_ERR "RAMDISK: Couldn't allocate lzma state.Probs\n");
	//printk("state.Properties.DictionarySize=%x\n", state.Properties.DictionarySize);
      	if (state.Properties.DictionarySize == 0)
    		state.Dictionary = 0;
  	else {
    		//state.Dictionary = (unsigned char *)kmalloc(state.Properties.DictionarySize, GFP_KERNEL);
    		state.Dictionary = (unsigned char *)__get_free_pages(GFP_KERNEL, 9); // 2^9 pages = 2MB
		if ( state.Dictionary == 0) {
			printk(KERN_ERR "RAMDISK: Couldn't allocate lzma state.Dictionary\n");
			return 0;
		}
	}
	//printk("LZMA: inLen=%x, outSize=%x, offset =%x, state.Probs=%p, state.Dictionary=%p, state.Properties.DictionarySize=%d\n", 
		//inLen, outSize, startBuf - ( unsigned char *)(inbuf), state.Probs, state.Dictionary, state.Properties.DictionarySize);
	...
	//printk(" outSize = %d, inAvail=%d, finishDecoding=%d\n", outSize, inAvail, finishDecoding);
	  res = LzmaDecode(&state,
	      inBuffer, inAvail, &inProcessed,
	      window, outAvail, &outProcessed, finishDecoding);
	//printk("inProcessed=%d, outProcessed=%d\n", inProcessed, outProcessed);
	  if (res != 0)
	  {
	    printk("\nDecoding error = %d\n", res);
	    res = 1;
	  ...
	  //printk(" outSize = %d, outProcessed=%d, inAvail=%d\n", outSize, outProcessed, inAvail);
	...
	free_pages(state.Dictionary, 9);
	free(state.Probs);
}

#else // !LZMA_COMPRESSED_RAMDISK
	makecrc();
	result = gunzip();
#endif	
	kfree(inbuf);
...
#endif  /* CONFIG_LOAD_COMPRESSED_RAMDISK */
