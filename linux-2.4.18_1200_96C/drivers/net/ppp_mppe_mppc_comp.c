	      ...
	      int isize, int osize)
{
    ...
    //since proto will not > 0xff, off should 2 such that mppe encrypt can get right value, for pptp iot issue
	...
	if ((complen > isize) || (complen == 0)) {
	    ...
	    olen = isize - (off - 2) + (PPP_HDRLEN / 2) + MPPE_OVHD;
	...
	if (!state->mppc && state->mppe) { /* Do only encryption */
	    ...
	    arcfour_encrypt(&state->arcfour_context, ibuf, isize - off, wptr);
		...
		unsigned char *obuf, int osize)
{
	...
	if ((bits & MPPE_BIT_ENCRYPTED)) {
		...
		uncomplen = mppc_decompress(state, ibuf+PPP_HDRLEN+MPPE_OVHD,
					    ...
					    osize-(PPP_HDRLEN/2));
#endif
		arcfour_decrypt(&state->arcfour_context, ibuf + PPP_HDRLEN +
				MPPE_OVHD, isize - PPP_HDRLEN - MPPE_OVHD,
...
//		       isize-PPP_HDRLEN-MPPE_OVHD);
		memcpy(obuf, ibuf + PPP_HDRLEN + MPPE_OVHD, isize - PPP_HDRLEN -
		       MPPE_OVHD);
		uncomplen = isize - (PPP_HDRLEN / 2) - MPPE_OVHD;
		(state->stats).inc_bytes += isize;
		(state->stats).inc_packets++;
