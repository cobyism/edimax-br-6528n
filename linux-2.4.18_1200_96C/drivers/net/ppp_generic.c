...
#ifdef CONFIG_RTL_KERNEL_MIPS16_DRVNET
...
extern int rtl865x_addPppInfoChannel(struct net_device *pppDev, struct ppp_channel *pppChan);
extern int rtl865x_delPppInfoChannel(struct net_device *pppDev, struct ppp_channel *pppChan);
extern int rtl865x_pppAcceleratePptpL2tpToWAN(struct sk_buff *skb, struct net_device *pppDev);
...
struct ppp {
	...
	int		mru_alloc;	/* tommy ==> MAX(1500,MRU) for dev_alloc_skb() */
	...
	int	xpad;			/* tommy ==> ECP or CCP (MPPE) transmit padding */
};

//brad add for pptp mppe rx out or order
#ifdef FAST_PPTP
...
#ifdef CONFIG_RTL865X_HW_PPTPL2TP
			if (!accelerate && fast_l2tp_to_wan(skb))
				return 0;
#else
...
#ifdef FAST_PPTP
	if (is_fast_fw) {
		//fastpptp: add 2 bytes for pptp iot issue--start
		skb_push(skb,2);
		skb->data[0] = PPP_ALLSTATIONS;
		skb->data[1] = PPP_UI;
		//fastpptp: add 2 bytes for pptp iot issue--end
...
#ifdef FAST_PPTP
void *get_ppp_vj(struct ppp *ppp)
{
	return ((void *)ppp->vj);
}
#ifdef CONFIG_FAST_PATH_MODULE
EXPORT_SYMBOL(get_ppp_vj);
#endif
unsigned int get_ppp_xstate(struct ppp *ppp)
{
	return ppp->xstate;
}
#ifdef CONFIG_FAST_PATH_MODULE
EXPORT_SYMBOL(get_ppp_xstate);
#endif
void *get_ppp_xc_state(struct ppp *ppp)
{
	return ((void *)ppp->xc_state);
}
#ifdef CONFIG_FAST_PATH_MODULE
EXPORT_SYMBOL(get_ppp_xc_state);
#endif
struct sk_buff *get_ppp_xmit_pending(struct ppp *ppp)
{

	return ppp->xmit_pending;
}
#ifdef CONFIG_FAST_PATH_MODULE
EXPORT_SYMBOL(get_ppp_xmit_pending);
#endif
void set_ppp_xmit_pending(struct ppp *ppp, struct sk_buff *skb)
{
	ppp->xmit_pending = skb;
}
#ifdef CONFIG_FAST_PATH_MODULE
EXPORT_SYMBOL(set_ppp_xmit_pending);
#endif
...
//fastpptp: shift 2 bytes for pptp iot issue--start
#ifdef FAST_PPTP
if(is_fast_fw)
proto = (((skb)->data[2] << 8) + (skb)->data[3]);
#endif
//fastpptp: shift 2 bytes for pptp iot issue--end
	...
	if (proto < 0x8000) {
		...
		*skb_push(skb, 2) = 1;
		...
		/*RTK:since data will be forward via fastpath, nfmark will not be update, the check will fail for ppp call on demand*/
		/*RTK:do not check nfmark for ppp xmit*/
...
#if defined(FAST_PPTP) && defined(NAT_SPEEDUP)
{
	...
	if ((ppp->xstate & SC_COMP_RUN) && ppp->xc_state != 0
		...
		int new_skb_size = ppp->dev->mtu + ppp->dev->hard_header_len + ppp->xpad + comp_ovhd;
		int compressor_skb_size = ppp->dev->mtu + PPP_HDRLEN + ppp->xpad;
	...
	#ifdef FAST_PPTP
		if(is_fast_fw)
			len = ppp->xcomp->compress(ppp->xc_state, skb->data , new_skb->data, skb->len, compressor_skb_size);
		else{
	...
	//fastpptp: shift 2 bytes for pptp iot issue--end
		if (len > 0 && (ppp->flags & SC_CCP_UP)) {
			...
			skb = new_skb;
			skb_put(skb, len);
	//fastpptp: do not pull off 2 bytes for pptp iot issue--start		
		#ifdef FAST_PPTP
			if (!is_fast_fw)
			skb_pull(skb, 2);	/* pull off A/C bytes */
				...
				skb_pull(skb, 2);	/* pull off A/C bytes */
		#endif
	//fastpptp: do not pull off 2 bytes for pptp iot issue--end	
		} else if (len == 0) {
...
#ifdef CONFIG_RTL865X_HW_PPTPL2TP
	if (accelerate && (rtl865x_pppAcceleratePptpL2tpToWAN(skb, ppp->dev) == 0)) {
		return;
	}
#endif /* CONFIG_RTL865X_HW_PPTPL2TP */
...
#ifdef CONFIG_RTL865X_HW_PPTPL2TP
			if (accelerate && skb->protocol == ETH_P_IP) {
				if (rtl865x_pppAcceleratePptpL2tpToLAN(skb, ppp->dev) < 0)
					goto err;
				else
					goto ok;
			}
...
static void ppp_destroy_interface(struct ppp *ppp)
{
	...
	up(&all_ppp_sem);
#ifdef CONFIG_RTL865X_HW_TABLES
	rtl865x_bind_pppoe(dev_name, NULL, 0);
#endif
}
...
ppp_connect_channel(struct channel *pch, int unit)
{
	...
	if (ppp->dev) {
		if (pch->pppoe)
			rtl865x_bind_pppoe(ppp->dev->name, pch->chan, 1);
#ifdef CONFIG_RTL865X_HW_PPTPL2TP
		else {
			rtl865x_addPppSession(ppp->dev->name, NULL, 0, 4 /* SE_PPP */);
			rtl865x_addPppInfoChannel(ppp->dev, pch->chan);
		}
#endif /* CONFIG_RTL865X_HW_PPTPL2TP */
	}
#endif						
...
ppp_disconnect_channel(struct channel *pch)
{
	...
	if (ppp != 0) {
		...
		if (ppp->dev) {
			if (pch->pppoe)
				rtl865x_bind_pppoe(ppp->dev->name, pch->chan, 0);
#ifdef CONFIG_RTL865X_HW_PPTPL2TP				
			else {
				rtl865x_delPppSession(ppp->dev->name,  4 /* SE_PPP */);
				rtl865x_delPppInfoChannel(ppp->dev, pch->chan);
			}
#endif /* CONFIG_RTL865X_HW_PPTPL2TP */
		}
#endif
