...
#if defined(_PPPOE_AUTO_PADT_)	// EDX SJC for Logitec
...
#endif
int rtl865x_bind_pppoe(char *name, struct pppox_opt *po, int add)
{
	int rc;

	//if (!name || (!po&&add))
	#ifdef CONFIG_FAST_PATH_MODULE
	if(FastPath_hook9!=NULL)
	{
		if (!name || (!po&&add) || FastPath_hook9())    /* if WAN type is PPTP, then return */
		return -1;	
	}
	#else
	if (!name || (!po&&add) || fast_pptp_fw)    /* if WAN type is PPTP, then return */
		return -1;				       
	#endif
	if (add) {
		if (!(po->sk->state&PPPOX_CONNECTED))
			return -1;
		rc = rtl865x_addPppoeSession(name, (void *)po->pppoe_pa.remote, po->pppoe_pa.sid, 0);
		LR_DEBUG("LR(%s): dev: %s, sid: %u, mac: %x:%x:%x:%x:%x:%x,  errno=%d\n",  
				"add_pppoe", name, po->pppoe_pa.sid, po->pppoe_pa.remote[0], po->pppoe_pa.remote[1], 
				po->pppoe_pa.remote[2], po->pppoe_pa.remote[3], po->pppoe_pa.remote[4], 
				po->pppoe_pa.remote[5], rc
		);
	}
	else {
		rc = rtl865x_delPppoeSession(name);
		LR_DEBUG("LR(%s): dev: %s, errno=%d\n", "del_pppoe", name, rc);
	}
	return (rc)? -1: 0;
}
#endif
...
static void sendPADT(struct sk_buff *skb, struct net_device *dev, struct packet_type *pt, u_short sid)
{
	...
	*/
	...
	//Fixed by 2008.10.01
	if (memcmp(skb->mac.ethernet->h_dest, dev->dev_addr,6))	// don't answer any packet which is not for me
		return;
	//End
	...
	dev->hard_header(skb2, dev, ETH_P_PPP_DISC,
             skb->mac.ethernet->h_source, NULL, data_len);
	//End
	dev_queue_xmit(skb2);
}
