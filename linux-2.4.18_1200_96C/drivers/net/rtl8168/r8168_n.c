...
#ifdef	RTL_IOCTL_SUPPORT
#include "rtl_ioctl.h"
#endif

#include <linux/compiler.h> //michael
...
static const struct {
	...
	_R("RTL8168B/8111B", CFG_METHOD_1, 0xff7e1880),
	_R("RTL8168B/8111B", CFG_METHOD_2, 0xff7e1880),	
	_R("RTL8168B/8111B", CFG_METHOD_3, 0xff7e1880),		
	_R("RTL8168C/8111C", CFG_METHOD_4, 0xff7e1880),		
	_R("RTL8168C/8111C", CFG_METHOD_5, 0xff7e1880),		
	_R("RTL8168CP/8111CP", CFG_METHOD_6, 0xff7e1880),		
	_R("RTL8168C/8111C", CFG_METHOD_7, 0xff7e1880),		
	_R("RTL8168CP/8111CP", CFG_METHOD_8, 0xff7e1880),		
	_R("RTL8168C/8111C", CFG_METHOD_9, 0xff7e1880),		
};
#undef _R
...
static const unsigned int rtl8168_rx_config =
...
static void 
rtl8168_gset_xmii(struct net_device *dev, 
		  struct ethtool_cmd *cmd)
{
	struct rtl8168_private *tp = netdev_priv(dev);
	void __iomem *ioaddr = tp->mmio_addr;
...
rtl8168_hw_phy_config(struct net_device *dev)
{
	...
	if (tp->mcfg == CFG_METHOD_1) {
		...
		mdio_write(ioaddr, 0x1F, 0x0000);
		mdio_write(ioaddr, 0x09, 0x2000);
		mdio_write(ioaddr, 0x09, 0x0000);
		...
		mdio_write(ioaddr, 0x1F, 0x0000);
		mdio_write(ioaddr, 0x1D, 0x0F00);

		mdio_write(ioaddr, 0x1F, 0x0002);
		mdio_write(ioaddr, 0x0C, 0x1EC8);
		mdio_write(ioaddr, 0x1F, 0x0000);
		 ...
		 int cmd)
{
	...
	default:
#ifdef RTL_IOCTL_SUPPORT
		return rtl8168_priv_ioctl(dev, ifr, cmd);
#else
		return -EOPNOTSUPP;		
#endif
	}
...
#ifdef RTL_IOCTL_SUPPORT
	tasklet_init(&tp->loopback_tasklet, rtl8168_loopback_test, (unsigned long)dev);
#endif 
...
rtl8168_open(struct net_device *dev)
{
	...
	retval = request_irq(dev->irq, rtl8168_interrupt, SA_SHIRQ, dev->name, dev);
#else
	retval = request_irq(dev->irq, rtl8168_interrupt, IRQF_SHARED, dev->name, dev);
#endif
...
rtl8168_hw_start(struct net_device *dev)
{
	...
	if (tp->mcfg == CFG_METHOD_4) {
		...
		RTL_W16(CPlusCmd, RTL_R16(CPlusCmd) & 
			~(EnableBist | Macdbgo_oe | Force_halpdup | Force_rxflow_en | Force_txflow_en | 
			  Cxpl_dbg_sel | ASF | PktCntrDisable | Macdbgo_sel));

		tp->max_jumbo_frame_size = Jumbo_Frame_9k;
		...
		/*Set EPHY register offset 0x03 bit 10 to 0, bit 5 to 1 and bit 9 to 1*/
		ephy_data = rtl8168_ephy_read(ioaddr, 0x03);
		ephy_data &= ~(1 << 10);
		ephy_data |= (1 << 5);
		ephy_data |= (1 << 9);
		   ...
		   int new_mtu)
{
	...
	if ((tp->mcfg == CFG_METHOD_4) ||
	    ...
	    (tp->mcfg == CFG_METHOD_8) ||
	    (tp->mcfg == CFG_METHOD_9)) {
		//for RTL8168C and RTL8168CP, tx max jumbo frame size is 6 kBytes.
		if (new_mtu > Jumbo_Frame_6k)
			dev->mtu = Jumbo_Frame_6k;
	} else if ((tp->mcfg == CFG_METHOD_1) || 
		   (tp->mcfg == CFG_METHOD_2)) {
		//for RTL8168B, tx max jumbo frame size is 4 kBytes.
		if (new_mtu > Jumbo_Frame_4k)
			dev->mtu = Jumbo_Frame_4k;
	}
...
#ifdef	RTL_IOCTL_SUPPORT
static int 
rtl8168_loopback_check_pkt(struct sk_buff *skb,
			   struct net_device *dev)
{
	struct rtl8168_private *tp = netdev_priv(dev);
	int i;
	int mac_head_len = MAC_ADDR_LEN * 2 + MAC_PROTOCOL_LEN;

	for (i = 0; i < tp->loopback_pkt_payload_len; i++)
		if (*(skb->data + mac_head_len + i) != *(tp->loopback_pkt_payload + i))
			break;

	if (i < tp->loopback_pkt_payload_len)
		return 1;
	else
		return 0;
}
#endif	//RTL_IOCTL_SUPPORT
...
#ifdef	RTL_IOCTL_SUPPORT
	if (tp->loopback_state == LOOPBACK_ON) {
		netif_stop_queue(dev);
		if (rtl8168_loopback_check_pkt(skb, dev))
...
#ifdef	RTL_IOCTL_SUPPORT
			if (tp->loopback_state == LOOPBACK_ON) {
				if (rtl8168_loopback_check_pkt(tp->Rx_skbuff[entry], dev))
					tp->loopback_rx_err++;
				else
					tp->loopback_rx_ok++;
...
rtl8168_close(struct net_device *dev)
{
	...
	if (tp->loopback_state == LOOPBACK_ON) {
		...
		udelay(1000);
		rtl8168_set_loopback(dev, tp->loopback_type, tp->loopback_state);
	}
	tasklet_kill(&tp->loopback_tasklet);
#endif
