...
/*****************************************************************************
 * porting from kernel 2.6 OSS architecture C-media driver to kernel 2.4
 * and merge RTL_WAA's delay function and add IOCTL for modify delay time
 * kfchang@realtek.com.tw
 ****************************************************************************/

#define RTL_WAA
#ifdef RTL_WAA
         static int delayed_start = 1;
#endif
...
//Modify to support 2.4 kernel by kfchang
# define __user
...
//kfchang
#define PREBUFTIME 100; // unit is mini-second(10^-3)
unsigned int prebuftime = PREBUFTIME;
...
//kfchang
static void set_prebuf(unsigned int time)
{
	prebuftime = time;
	if (time >=0)
		delayed_start = 1;	
...
#ifdef RTL_WAA
        printk("\n\nbufs = %d\n", bufs);
        printk("db->fragsize = %d\n", db->fragsize);
        printk("db->numfrag = %d\n", db->numfrag);
        printk("db->fragshift = %d\n", db->fragshift);
        printk("db->fragsamples = %d\n", db->fragsamples);
        printk("db->dmasize = %d\n", db->dmasize);
        printk("db->dmasamples = %d\n", db->dmasamples);
#endif
...
static int mixer_ioctl(struct cm_state *s, unsigned int cmd, unsigned long arg)
{
	...
	VALIDATE_STATE(s);
		...
		//change strlcpy --> strncpy, by kfchang
...
#ifdef RTL_WAA
                if ( delayed_start &&
                   (s->dma_dac.count >= s->dma_dac.dmasize
                   || (s->dma_dac.count) >= prebuftime*176400)) { 
                        delayed_start = 0;
                        printk("delayed_start = 1, (%d) \n",s->dma_dac.count);
                }
                if (!delayed_start)
                        start_dac(s);
#else
...
#define SNDCTL_PREBUF		_SIOW('S',  9, int)       // set PREBUF by kfchang
