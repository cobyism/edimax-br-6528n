...
#define RTL_USB_ADD 0
#define RTL_USB_REMOVE 1
extern void toggle_usb_device_insert_bit(void);
extern void toggle_usb_device_remove_bit(void);
void rtl_usb_detect(int action ,struct usb_device *dev);
#endif
...
#ifdef USB_FINETUNE
#define __USB_IRAM_SECTION_ __attribute__ ((section(".iram-fwd")))
#endif
			 ...
			 __u16 value, __u16 index, void *data, __u16 size, int timeout)
{
	...
	//dbg("usb_control_msg");	
#ifdef CONFIG_RTL8197B_PANA	
	if( requesttype == 0xc0 ) //0xc0 is a (request in) control in rxxxk wlan driver  
	{		
		if( (size == 2) && (((unsigned int)(data) & 0x00000003) != 0))//if not 4-byte alignment
		{
			unsigned int data_tmp;
			void *data_tmp_p = &data_tmp;			
			ret = usb_internal_control_msg(dev, pipe, dr, data_tmp_p, size, timeout);
			*((unsigned short *)data) = (unsigned short)(data_tmp >> 16);
			goto out;	
		}
	}	
#endif
...
void usb_disconnect(struct usb_device **pdev)
{
	...
	*pdev = NULL;
#if	(!defined(CONFIG_USB_OHCI_RTL865XB) && !defined(CONFIG_USB_EHCI_HCD_RTL8652))
...
#ifdef CONFIG_RTL8197B_PANA
	rtl_usb_detect(RTL_USB_REMOVE,dev);
#endif
...
void rtl_usb_detect(int action ,struct usb_device *dev)
{
	if( dev->descriptor.bDeviceClass == 0x09 ) //hub class code
		return;
	//printk("USB action=%d\n",action); //mark_test
	if(action == RTL_USB_ADD)
	    toggle_usb_device_insert_bit();
	else if(action == RTL_USB_REMOVE)
	    toggle_usb_device_remove_bit();				
	return;	
}
#endif
/*
...
void usb_connect(struct usb_device *dev)
{
	...
	/*  tony: fixed for NULL access issue */
	if(dev->bus==NULL) { printk("dev->bus is NULL\n"); return; }
...
#ifdef CONFIG_RTL8197B_PANA
	rtl_usb_detect(RTL_USB_ADD,dev);
#endif
