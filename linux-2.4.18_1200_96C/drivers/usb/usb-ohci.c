...
// #define OHCI_VERBOSE_DEBUG	1 /* not always helpful */
...
u32 temp_virt[RSIZE];
u32 temp_hw[RSIZE];
u32 temp_HeadP[RSIZE];
u32 temp_TailP[RSIZE];
u32 temp_ed[RSIZE];
u32 grx=0;
#define NEXT_RX(N)		(((N) + 1) & (RSIZE - 1))
#define PREV_RX(N)		(((N) - 1) & (RSIZE - 1))
...
#define OHCI_UNLINK_TIMEOUT	(HZ * 30)
...
int hc_interrupt (int irq, void * __ohci, struct pt_regs * r);
/*-------------------------------------------------------------------------*
...
#if 0
	temp_virt[grx] = (u32)td;
	temp_ed[grx] = (u32)urb_priv->ed;
	temp_HeadP[grx] = (u32)urb_priv->ed->hwHeadP;
	temp_TailP[grx] = (u32)urb_priv->ed->hwTailP;
	temp_hw[grx] = (u32)info;
#endif
	td->ed = urb_priv->ed;
	td->next_dl_td = NULL;
	td->index = index;
...
static td_t * dl_reverse_done_list (ohci_t * ohci)
{
	...
	dbg_td_list = td_list_hc;
#if 0
	if ((td_list_hc > 0x800000) || (td_list_hc &0xf))
	{
		printk(" once %d line %d td_list_hc %x\n",once,__LINE__,td_list_hc);
		//td_list_hc = be32_to_cpup (&ohci->hcca->done_head) & 0xfffffff0;
		//if ((td_list_hc > 0x800000) || (td_list_hc &0xf))
		for(;;);
	}
	...
	while (td_list_hc) {		
		...
		if (TD_CC_GET (be32_to_cpup (&td_list->hwINFO))) {
			...
			printk("USB-error /status value: %x : %p\n", 
		...
		if ((td_list_hc > 0x800000) || (td_list_hc &0xf))
		{
			printk(" once %d line %d next td_list_hc %x\n",once,__LINE__,td_list_hc);
			for(;;);
	}
#endif
	}	
...
#endif	/* OHCI_USE_NPS */
	writel(0x32001204,&ohci->regs->roothub.a);
...
hc_found_ohci (int irq, void *mem_base)
{
	ohci_t * ohci;
	char buf[8];
	int ret;
...
//	writel (ohci->hc_control = OHCI_USB_RESET, &ohci->regs->control);
	...
	ohci->irq = irq=ICU_USB;     
	if (request_irq (irq, hc_interrupt, SA_INTERRUPT,
			"rtl865x usb driver", ohci) != 0) {
		hc_release_ohci (ohci);
		return -EBUSY;
	}
