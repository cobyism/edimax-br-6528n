...
static  dma_addr_t rtl_map_single(void *ptr,
...
static void urb_free_priv (struct ohci *hc, urb_priv_t * urb_priv)
{
	...
	if (last >= 0) {
		...
		if (usb_pipecontrol (td->urb->pipe)) {
			rtl_unmap_single (td->data_dma, 8, PCI_DMA_TODEVICE);
...
void ep_print_int_eds (ohci_t * ohci, char * str) {
	...
	for (i= 0; i < 32; i++) {
		...
		while (*ed_p != 0 && j--) {
			ed_t *ed = dma_to_ed (ohci, be32_to_cpup(ed_p));
...
static int sohci_return_urb (struct ohci *hc, urb_t * urb)
{
	...
	switch (usb_pipetype (urb->pipe)) {
  		case PIPE_INTERRUPT:
			rtl_unmap_single (urb_priv->td [0]->data_dma,
				urb->transfer_buffer_length,
		...
		case PIPE_ISOCHRONOUS:
			if (urb->transfer_flags & USB_ISO_ASAP) { 
				urb->start_frame = ((ed->state == ED_OPER)
					? (ed->last_iso + 1)
					: (be16_to_cpu (ohci->hcca->frame_no) + 10)) & 0xffff;
			}	
			/* FALLTHROUGH */
		case PIPE_INTERRUPT:
			if (urb->bandwidth == 0) {
	...
	if (usb_dev->devnum >= 0) {
		...
		for(i = 0; i < NUM_EDS; i++) {
  			...
  			if (ed->state != ED_NEW) {
  				if (ed->state == ED_OPER) {
					...
					dbg ("driver usb dev %d ed 0x%x unfreed URB", usb_dev->devnum, i);
					ep_unlink (ohci, ed);
				}
  				ep_rm_ed (usb_dev, ed);
  				ed->state = ED_DEL;
		...
		if (cnt > 0) {
			...
			if (ohci->disabled) {
				 ...
				 * these lists again!!
				dl_del_list (ohci,
					be16_to_cpu (ohci->hcca->frame_no) & 1);
				 */
				warn ("TD leak, %d", cnt);
...
static int sohci_get_current_frame_number (struct usb_device *usb_dev) 
{
	...
	return be16_to_cpu (ohci->hcca->frame_no);
}
...
{	 
	...
	case PIPE_CONTROL:
		...
		if (ohci->ed_controltail == NULL) {
			...
			ohci->ed_controltail->hwNextED = cpu_to_be32 (ed->dma);
		}
	...
	case PIPE_BULK:
		...
		if (ohci->ed_bulktail == NULL) {
			...
			ohci->ed_bulktail->hwNextED = cpu_to_be32 (ed->dma);
		}
	...
	case PIPE_INTERRUPT:
		...
		for (i = 0; i < ep_rev (6, interval); i += inter) {
			...
			for (ed_p = &(ohci->hcca->int_table[ep_rev (5, i) + int_branch]); 
				(*ed_p != 0) && ((dma_to_ed (ohci, be32_to_cpup (ed_p)))->int_interval >= interval); 
				ed_p = &((dma_to_ed (ohci, be32_to_cpup (ed_p)))->hwNextED)) 
					inter = ep_rev (6, (dma_to_ed (ohci, be32_to_cpup (ed_p)))->int_interval);
			ed->hwNextED = *ed_p; 
			*ed_p = cpu_to_be32 (ed->dma);
		}
#ifdef DEBUG
	...
	case PIPE_ISOCHRONOUS:
		...
		if (ohci->ed_isotail != NULL) {
			ohci->ed_isotail->hwNextED = cpu_to_be32 (ed->dma);
...
static void periodic_unlink (
	...
	for (; index < NUM_INTS; index += period) {
		...
		while (*ed_p != 0) {
			if ((dma_to_ed (ohci, be32_to_cpup (ed_p))) == ed) {
				*ed_p = ed->hwNextED;		
				break;
			}
			ed_p = & ((dma_to_ed (ohci,
				be32_to_cpup (ed_p)))->hwNextED);
		}
	}	
}
...
static int ep_unlink (ohci_t * ohci, ed_t * ed) 
{
	...
	ed->hwINFO |= cpu_to_be32 (OHCI_ED_SKIP);
	...
	case PIPE_CONTROL:
		if (ed->ed_prev == NULL) {
			...
			writel (be32_to_cpup (&ed->hwNextED), &ohci->regs->ed_controlhead);
		} else {
		...
		if (ohci->ed_controltail == ed) {
			...
			(dma_to_ed (ohci, be32_to_cpup (&ed->hwNextED)))->ed_prev = ed->ed_prev;
		}
		break;
	...
	case PIPE_BULK:
		if (ed->ed_prev == NULL) {
			...
			writel (be32_to_cpup (&ed->hwNextED), &ohci->regs->ed_bulkhead);
		} else {
			ed->ed_prev->hwNextED = ed->hwNextED;
		}
	...
	case PIPE_ISOCHRONOUS:
		...
		if (ed->hwNextED != 0) 
		    (dma_to_ed (ohci, be32_to_cpup (&ed->hwNextED)))
		    	->ed_prev = ed->ed_prev;
...
)
{
	...
	if (ed->state == ED_NEW) {
		ed->hwINFO = cpu_to_be32 (OHCI_ED_SKIP); /* skip ed */
		...
		ed->hwTailP = cpu_to_be32 (td->td_dma);
	...
	ed->hwINFO = cpu_to_be32 (usb_pipedevice (pipe)
...
{    
	...
	frame = be16_to_cpu (ohci->hcca->frame_no) & 0x1;
	...
	urb_t * urb, int index)
{
	...
	td = urb_priv->td [index] = dma_to_td (ohci,
			be32_to_cpup (&urb_priv->ed->hwTailP) & ~0xf);
	...
	td->hwINFO = cpu_to_be32 (info);
	if ((td->ed->type) == PIPE_ISOCHRONOUS) {
		td->hwCBP = cpu_to_be32 (data & 0xFFFFF000);
		td->ed->last_iso = info & 0xffff;
	} else {
		td->hwCBP = cpu_to_be32 (data); 
	}			
	if (data)
		td->hwBE = cpu_to_be32 (data + len - 1);
	else
		td->hwBE = 0;
	td->hwNextTD = cpu_to_be32 (td_pt->td_dma);
	td->hwPSW [0] = cpu_to_be16 ((data & 0x0FFF) | 0xE000);
...
{ 
	...
	if (data_len) {
		data = rtl_map_single (urb->transfer_buffer, data_len,
			usb_pipeout (urb->pipe)
				? PCI_DMA_TODEVICE
				: PCI_DMA_FROMDEVICE
			);
	...
	switch (usb_pipetype (urb->pipe)) {
		...
		case PIPE_CONTROL:
			...
			//info = TD_CC | TD_DP_SETUP | TD_T_DATA0  
			td_fill (ohci, info,
				rtl_map_single (urb->setup_packet, 8,
					PCI_DMA_TODEVICE),
				8, urb, cnt++); 
			if (data_len > 0) {  
				info = usb_pipeout (urb->pipe)? 
...
static void dl_transfer_length(td_t * td)
{
	...
	tdINFO = be32_to_cpup (&td->hwINFO);
  	tdBE   = be32_to_cpup (&td->hwBE);
  	tdCBP  = be32_to_cpup (&td->hwCBP);


  	if (tdINFO & TD_ISO) {
 		tdPSW = be16_to_cpu (td->hwPSW[0]);
 		cc = (tdPSW >> 12) & 0xF;
		if (cc < 0xE)  {
...
td_t * dl_reverse_done_list (ohci_t * ohci)
{
	...
	td_list_hc = be32_to_cpup (&ohci->hcca->done_head) & 0xfffffff0;
	ohci->hcca->done_head = 0;
	
	while (td_list_hc) {		
		td_list = dma_to_td (ohci, td_list_hc);

		if (TD_CC_GET (be32_to_cpup (&td_list->hwINFO))) {
			urb_priv = (urb_priv_t *) td_list->urb->hcpriv;
			dbg(" USB-error/status: %x : %p", 
					TD_CC_GET (be32_to_cpup (&td_list->hwINFO)), td_list);
			if (td_list->ed->hwHeadP & cpu_to_be32 (0x1)) {
				if (urb_priv && ((td_list->index + 1) < urb_priv->length)) {
					td_list->ed->hwHeadP = 
						(urb_priv->td[urb_priv->length - 1]->hwNextTD & cpu_to_be32 (0xfffffff0)) |
									(td_list->ed->hwHeadP & cpu_to_be32 (0x2));
					urb_priv->td_cnt += urb_priv->length - td_list->index - 1;
				} else 
					td_list->ed->hwHeadP &= cpu_to_be32 (0xfffffff2);
			}
		}

		td_list->next_dl_td = td_rev;	
		td_rev = td_list;
		td_list_hc = be32_to_cpup (&td_list->hwNextTD) & 0xfffffff0;	
	}	
	spin_unlock_irqrestore (&usb_ed_lock, flags);
	return td_list;
}
...
static void dl_del_list (ohci_t  * ohci, unsigned int frame)
{
	...
	for (ed = ohci->ed_rm_list[frame]; ed != NULL; ed = ed->ed_rm_list) {
		...
		tdTailP = dma_to_td (ohci, be32_to_cpup (&ed->hwTailP) & 0xfffffff0);
		tdHeadP = dma_to_td (ohci, be32_to_cpup (&ed->hwHeadP) & 0xfffffff0);
		edINFO = be32_to_cpup (&ed->hwINFO);
		td_p = &ed->hwHeadP;
		...
		for (td = tdHeadP; td != tdTailP; td = td_next) { 
			...
			td_next = dma_to_td (ohci, be32_to_cpup (&td->hwNextTD) & 0xfffffff0);
			if ((urb_priv->state == URB_DEL) || (ed->state & ED_DEL)) {
				tdINFO = be32_to_cpup (&td->hwINFO);
				if (TD_CC_GET (tdINFO) < 0xE)
					dl_transfer_length (td);
				*td_p = td->hwNextTD | (*td_p & cpu_to_be32 (0x3));
	...
	urb_priv_t * urb_priv;
 	...
 	unsigned long flags;
  	...
  	while (td_list) {
  		...
  		urb = td_list->urb;
  		urb_priv = urb->hcpriv;
  		tdINFO = be32_to_cpup (&td_list->hwINFO);
  		...
  		spin_lock_irqsave (&usb_ed_lock, flags);
  		if (ed->state != ED_NEW) { 
  			edHeadP = be32_to_cpup (&ed->hwHeadP) & 0xfffffff0;
  			edTailP = be32_to_cpup (&ed->hwTailP);
...
static int rh_submit_urb (urb_t * urb)
{
	...
	wValue        = be16_to_cpu (cmd->value);
	wIndex        = be16_to_cpu (cmd->index);
	wLength       = be16_to_cpu (cmd->length);
...
#else
	fminterval = (u32)(ohci->regs->fminterval & 0x3fff);
	fminterval |= (0x2778 << 16);
	printk ( "fminterval %d %x\n",fminterval,fminterval);
	//ohci->HcRegs->PeriodicStart = (0x2edf*9)/10;
	writel ( (0x2edf*9)/10,&ohci->regs->periodicstart);
#endif
 	/* start controller operations */
 	/*ohci->hc_control = OHCI_CONTROL_INIT | OHCI_USB_OPER;*/
 	ohci->hc_control = 0xb8;
	ohci->disabled = 0;
 	writel (ohci->hc_control, &ohci->regs->control);
 
	/* Choose the interrupts we care about now, others later on demand */
	mask = OHCI_INTR_MIE | OHCI_INTR_UE | OHCI_INTR_WDH | OHCI_INTR_SO|OHCI_INTR_RHSC | OHCI_INTR_FNO;	
...
void hc_interrupt (int irq, void * __ohci, struct pt_regs * r)
{
	//ohci_t * ohci = __ohci;
	if (!dbg_ohci) return;
	ohci_t * ohci = dbg_ohci;
	struct ohci_regs * regs = ohci->regs;
 	int ints; 

	if ((ohci->hcca->done_head != 0) && !(be32_to_cpup (&ohci->hcca->done_head) & 0x01)) {
		ints =  OHCI_INTR_WDH;
	} else if ((ints = (readl (&regs->intrstatus) & readl (&regs->intrenable))) == 0) {

		//writel (readl(&regs->intrstatus), &regs->intrstatus); 	 
		return;
	} 
	//spin_lock(int_lock);
	// dbg("Interrupt: %x frame: %x", ints, be16_to_cpu (ohci->hcca->frame_no));
	...
	if (ints & OHCI_INTR_SF) { 
		unsigned int frame = be16_to_cpu (ohci->hcca->frame_no) & 1;
	...
	void *mem_base)
{
	...
	bufp = __irq_itoa(irq);
#endif
	printk(__FILE__ ": USB OHCI at membase 0x%lx, IRQ %s\n",
...
ohci_probe ()
{
	...
	*(volatile u32*)0xbc803004=0x80000000;

	/* controller writes into our memory */
	status = hc_found_ohci (ICU_USB, mem_base);
	if (status < 0) {
		iounmap (mem_base);
	}
	return status;
