...
//	u16 fc = le16_to_cpu(header->frame_ctl);
	...
	case IW_MODE_ADHOC:
	{
		...
		for (p = (&ieee->ibss_mac_hash[index])->next; p != (&ieee->ibss_mac_hash[index]); p = p->next) {
		 ...
		 struct ieee80211_rx_stats *rx_stats)
{
	...
	int retval;	
	nat25_rx_handle(skb);
	/*if dest MAC is not myself should accelerate*/
	if(memcmp(skb->data,dev->dev_addr,dev->addr_len))
	{
		//dump_packet(skb,"RX");
		/*printk("%2x:%2x:%2x:%2x:%2x:%2x  add_len %d\n",dev->dev_addr[0],dev->dev_addr[1],dev->dev_addr[2],dev->dev_addr[3],
		dev->dev_addr[4],dev->dev_addr[5],dev->addr_len);*/
		retval =rtl8187_wlanAccelerate(dev, skb);
		if(0== retval){	
		    goto rx_exit;
		}
		else if(-1 == retval)
	...
	printk("%s %d\n",__FUNCTION__,__LINE__);
		...
		IEEE80211_DEBUG_SCAN("Updating '%s' (" MAC_FMT ") via %s.\n",
				     escape_essid(target->ssid,
						  target->ssid_len),
				     MAC_ARG(target->bssid),
				     WLAN_FC_GET_STYPE(le16_to_cpu(beacon->header.frame_ctl)) ==
				     IEEE80211_STYPE_PROBE_RESP ?
				     "PROBE RESPONSE" : "BEACON");
#ifdef DEBUG_RX
	printk("%s %d\n",__FUNCTION__,__LINE__);
...
{
#ifdef DEBUG_RX
	printk("%s %d frame_ctl %x\n",__FUNCTION__,__LINE__,header->frame_ctl);
#endif
