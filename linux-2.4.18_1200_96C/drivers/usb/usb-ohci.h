...
static  void *
dma_to_ed_td1 (struct hash_list_t * entry, dma_addr_t dma)
{
	int i;
	struct hash_t * scan = entry->head;
...
{
	//printk("%s %d   hash %d \n",__FILE__,__LINE__,TD_HASH_FUNC(td_dma));
	return (struct td *) dma_to_ed_td1(&(hc->td_hash[TD_HASH_FUNC(td_dma)]),
				      td_dma);
}
