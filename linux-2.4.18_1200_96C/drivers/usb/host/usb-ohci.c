...
static void dump_td_of_urb(urb_priv_t * urb_priv)
{
	int i;
	int		last = urb_priv->length - 1;
...
static void dump_urb (struct urb *purb)
{
	...
	{
		ohci = (ohci_t *)purb->dev->bus->hcpriv;
		if (usb_pipedevice (pipe) == ohci->rh.devnum) 
			dbg("root hub urb");
	}
	dbg("urb                   :%p", purb);
	dbg("hcpriv               :%p",purb->hcpriv);
...
static void dump_urb(struct urb * urb)
{
	...
	int i,len;
	dbg("urb 0x%x hcpriv 0x%x",urb, urb->hcpriv);
	if (!urb->dev || !urb->dev->bus) {
		dbg("\nURB: no dev");
		return;
	}
	if (urb->status != 0)
	dbg(" URB:[%4x] dev:%2d,ep:%2d-%c,type:%s,flags:%4x,len:%d/%d,stat:%d(%x)", 
...
#endif
static void dump_ed(ed_t * ed)
{
	dbg("\nED addres 0x%x size %d",(int)ed, sizeof(*ed));
	
		dbg("hwINFO 0x%x", le32_to_cpup(&ed->hwINFO));
		dbg("hwTailP 0x%x",le32_to_cpup(&ed->hwTailP));
		dbg("hwHeadP 0x%x",le32_to_cpup(&ed->hwHeadP));
		dbg("hwNextED0x%x",le32_to_cpup(&ed->hwNextED));

		dbg("ed_prev 0x%x",ed->ed_prev);
		dbg("int_period 0x%x",ed->int_period);
		dbg("int_branch 0x%x",ed->int_branch);
		dbg("int_load 0x%x", ed->int_load);
		dbg("int_interval 0x%x",ed->int_interval);
		dbg("state 0x%x",ed->state);
		dbg("type 0x%x",ed->type);
		dbg("last_iso 0x%x",ed->last_iso);
}
static void dump_td(td_t * td_list)
{

	dbg("\nTD Start");
	dbg("td addres 0x%x size %d",(int)td_list, sizeof(*td_list));
	dbg("hwINFO 0x%x",le32_to_cpup(&td_list->hwINFO));
	dbg("Current Buffer Pointer 0x%x",le32_to_cpup(&td_list->hwCBP));
	dbg("Next TD Pointer  0x%x",le32_to_cpup(&td_list->hwNextTD));
	dbg(" Memory Buffer End Pointer  0x%x",le32_to_cpup(&td_list->hwBE));
	...
	if(td_list->urb)
	{
		dump_urb(td_list->urb);
	}
	dbg("TD End\n");
}

static void dump_td_list(td_t * td_list)
{
  	td_t * td_list_next = td_list;
	while(td_list_next)
	{
		dump_td(td_list_next);
		td_list_next = td_list_next->next_dl_td;
	}
}
...
void ep_print_ctrl_eds (ohci_t * ohci, char * str) 
{

	 volatile ed_t * ed = ohci->ed_controltail;
	 dbg("%s reversed",str);
	while(ed) {
		dbg("ed 0x%p",ed);
		ed = ed->ed_prev;
