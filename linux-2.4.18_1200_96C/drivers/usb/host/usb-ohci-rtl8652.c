...
/*USB Bus is little endian.since readl and writel is mapping to the pci read/write
   if PCI Bus is in Big endian,usb won't work.here redefine readl and writel.
   our cpu is default Big endian mode.
*/
/*I/O Mapping*/
#ifdef readl
#undef readl
...
#undef writel
#endif
/*let kernel handle cpu endian. be sure all readl and writl use new definitions
   in usb driver*/
#define readl(x) (le32_to_cpu((*(volatile unsigned long *)(x))))
#define writel(v,addr) ((*(volatile unsigned long *)(addr))=cpu_to_le32(v))
...
/*	mask = OHCI_INTR_MIE | OHCI_INTR_UE | OHCI_INTR_WDH | OHCI_INTR_SO | OHCI_INTR_SF;  */
...
/* allocate OHCI */
static char busname[10] = "onchip\0";
...
#if 0 /* test ehci SOF */
int i;
//unsigned int base=0xbbf03000,op_off=0x20,port=3;
unsigned int base=0xb8021000,op_off=0x10,port=0;

//USBCMD
REG32(base+op_off+0)=le32_to_cpu(0x00080000);

//USBSTS
REG32(base+op_off+4)=le32_to_cpu(0x00001000);

//USBINTR 
REG32(base+op_off+8)=le32_to_cpu(0x0);

//FRINDEX
REG32(base+op_off+0xc)=le32_to_cpu(0x0);

//CTRLDSSEGMENT
REG32(base+op_off+0x10)=le32_to_cpu(0x0);


//CONFIGFLAG
REG32(base+op_off+0x40)=le32_to_cpu(0x0);

//PORTSC
REG32(base+op_off+0x44+port*4)=le32_to_cpu(0x2000);


// action 1
// dont care the CTRLDSSEGMENT

// action 2
//USBINTR : Host System Error , Port Change Interrupt , USB Error Interrupt, USB Interrupt
REG32(base+op_off+8)=le32_to_cpu(0x17);

// action 3
// PERIODICLISTBASE
memset(0x80800000,0,1024);
for(i=0;i<1024;i++)
	REG32(0x80800000+i*4)=1;

REG32(base+op_off+0x14)=le32_to_cpu(0x00800000);
// ASYNCLISTADDR
memset(0x81000000,0,1024);
REG32(base+op_off+0x18)=le32_to_cpu(0x01000000);

// action 4
//USBCMD
REG32(base+op_off+0)=le32_to_cpu(0x00080001);

// action 5
//CONFIGFLAG
REG32(base+op_off+0x40)=le32_to_cpu(0x1);


while(1)
{

	// Port 0: Port Stauts and Control Register (Port Reset / make sure "PortEnabled" to zero)
	// must keep Port Reset bit keep in 1 for 2ms.
	REG32(base+op_off+0x44+port*4) = le32_to_cpu(0x1100);  

	printk("Port Status and Control Register =%x\n",le32_to_cpu(REG32(base+op_off+0x44+port*4)));

	for(i=0;i<300000;) { i++; } //wait 2ms
	for(i=0;i<300000;) { i++; } //wait 2ms
	for(i=0;i<300000;) { i++; } //wait 2ms
	for(i=0;i<300000;) { i++; } //wait 2ms
	for(i=0;i<300000;) { i++; } //wait 2ms
	...
	// Port 0: Port Stauts and Control Register ( clear Port reset bit)
	REG32(base+op_off+0x44+port*4) = le32_to_cpu(0x1000);


	for(i=0;i<=0x70;i+=4)
	{
		printk("offset %x = 0x%x\n",i,le32_to_cpu(REG32(base+i)));
	}

	if((le32_to_cpu(REG32(base+op_off+0x44+port*4))&4)==4) break; // Port Enable
}
printk("Final Port Status and Control Register =%x\n",le32_to_cpu(REG32(base+op_off+0x44+port*4)));
	return 0;
#else
