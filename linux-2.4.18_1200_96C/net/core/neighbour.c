...
static int syn_asic_arp(struct neighbour *n, char *fun, int add)
{
	...
	rc = add? rtl865x_addArp(arp_ip, (void *)n->ha, 0): rtl865x_delArp(arp_ip);
	LR_DEBUG("LR(%s_arp): ip:%u.%u.%u.%u, mac:%x:%x:%x:%x:%x:%x, errno=%d\n", 
			((add)? "add": "del"), NIPQUAD(arp_ip), n->ha[0], n->ha[1], 
			n->ha[2], n->ha[3], n->ha[4], n->ha[5], rc
	);
	return 0;
}
#endif
