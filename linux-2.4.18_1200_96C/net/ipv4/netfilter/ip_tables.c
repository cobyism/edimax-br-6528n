		...
		for (;;) {
			...
			if (e->target_offset == sizeof(struct ipt_entry)
...
#ifdef CONFIG_RTL865X_HW_TABLES
				if (masq_if[0] == '\0' && hook == NF_IP_POST_ROUTING &&
					strcmp(t->target.u.user.name, "MASQUERADE") == 0) 
				{
					...
					dev = dev_get_by_name(masq_if);
					if (dev&&dev->ip_ptr) {
						if ((ina=((struct in_device *)dev->ip_ptr)->ifa_list)) {
							if (hw_napt_ip) {
								rc = rtl865x_delNaptIp(hw_napt_ip);
								LR_DEBUG("LR(%s): 0x%x, errno=%d\n", "del_natip2", hw_napt_ip, rc);
							}
...
#ifdef CONFIG_RTL865X_HW_TABLES
		if (masqif_set == 0 && masq_if[0] != '\0' && hook==NF_IP_POST_ROUTING) {
			masq_if[0] = '\0';
		}
#endif	
	}
