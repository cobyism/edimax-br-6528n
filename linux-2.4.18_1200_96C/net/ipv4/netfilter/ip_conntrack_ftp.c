...
static char ftpalg_count=7;
static char ftpalg_flag[1024];
unsigned char ftpalg_entryenabled;
unsigned short ftpalg_portnum;
...
static int ftpalg_read_proc(char *page, char **start, off_t off,
...
static int ftpalg_write_proc(struct file *file, const char *buffer,
		      unsigned long count, void *data)
{
      char tmpbuf[1024];
      char *tokptr, *strptr=tmpbuf;
	u_int8_t idx=0;
	extern int ftp_alg_update_conntrack(unsigned short ftp_port_user, int action);
	extern int ftp_alg_update_nat(unsigned short ftp_port_user, int action);
	
      if (count < 2)
	    return -EFAULT;
      if (buffer && !copy_from_user(&ftpalg_flag, buffer, count)) {
	      strncpy(tmpbuf,ftpalg_flag,count);
	      tmpbuf[strlen(ftpalg_flag)]='\0';
	      tokptr = strtok(strptr," ");
              strptr=NULL;
              ftpalg_entryenabled=simple_strtol(tokptr,NULL,0);
	      while (idx< ftpalg_count)
	      {
	      	if((tokptr = strtok(strptr," "))==NULL)
	        	break;
              	strptr=NULL;
	      	ftpalg_portnum = simple_strtol(tokptr,NULL,0);
	      	idx++;
	      }
	      //printk("ftp alg update enable=%d\n", ftpalg_entryenabled);
	      //printk("ftp alg update port number=%d\n", ftpalg_portnum);
      		if(ftpalg_entryenabled ==1){
      			ftp_alg_update_conntrack(ftpalg_portnum, 1);
      			ftp_alg_update_nat(ftpalg_portnum, 1);
      		}else if(ftpalg_entryenabled ==0){
      			ftp_alg_update_conntrack(ftpalg_portnum, 2);
      			ftp_alg_update_nat(ftpalg_portnum, 2);
      		}
...
#ifdef  FTP_CONNTRACK_NAT_UPDATE
      if (resb) {
		remove_proc_entry("ftpalg_update", resb);
		resb = NULL;
      }
#endif //FTP_CONNTRACK_NAT_UPDATE
#endif //CONFIG_PROC_FS
...
#if defined(CONFIG_PROC_FS)	
#ifdef FTP_CONNTRACK_NAT_UPDATE
	resb = create_proc_entry("ftpalg_update", 0, NULL);
	if (resb) {
...
#ifdef FTP_CONNTRACK_NAT_UPDATE
int ftp_alg_update_conntrack(unsigned short ftp_port_user, int action)
{
	int i, ret;
	char *tmpname;
	int ftp_index=0;
